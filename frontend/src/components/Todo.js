...
import { Link } from 'react-router-dom';
  ...
  addTodo = (index, open, new_task, values) => {
    console.log(this.state.tasks)
    if (values == null) {
      values = {'task_title':'', 'duration':'','priority':'','category_id':'','constraints':''}
    }
    let i = index
    let sign;
    if (open) {
      sign = <svg id='expand_icon' onClick={() =>  this.addTodo(index, !open, false)} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="rgb(12, 13, 13)" width="30px" height="30px"><path d="M0 0h24v24H0z" fill="none"/>
      ...
      sign = <svg id='expand_icon' name='plus' onClick={() =>  this.addTodo(index, !open, false)} key='plus_sign' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="rgb(12, 13, 13)" width="30px" height="30px">
        <path d="M13 7h-2v4H7v2h4v4h2v-4h4v-2h-4V7zm-1-5C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"/>
      </svg>;
    }
    // let trash_bin = <svg style={{float:"right", marginTop:"inherit"}} onClick={() => this.remove_task(index)} height="30px" viewBox="-40 0 427 427.00131" fill="rgb(12, 13, 13)" width="30px" xmlns="http://www.w3.org/2000/svg">
    ...
    // let expand_task = <span style={{float:"left", height:"100%"}} key='expand_task'>{trash_bin}{sign} Title:</span>;
    // let expand_task = <span style={{float:"left"}}>Title:</span>;
    ...
    fetch("http://localhost:5000/gethello")
        ...
        .then(
            (result) => {
              ...
              for (let i=0; i<5; i++){
                this.addTodo(this.state.task_number, false, true, result[i])
              }
              this.setState({
                isLoaded: true,
  ...
  componentDidMount() {
    this.dataButtonHandler()
    let that = this
    setTimeout(function() {
      that.addTodo(that.state.task_number, false, true)
    },300)
  }
        ...
        <div>
          ...
          <header className="App-header">
            <form id='test' onSubmit={this.onSubmitHandler}>
              ...
              <div>
                <div onClick={() => this.addTodo(this.state.task_number, false, true)}>Add new task</div>
              </div>
